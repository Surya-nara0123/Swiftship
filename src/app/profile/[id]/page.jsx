...
export default function Page({ params }) {
  ...
  const getOrders = async () => {
    ...
    try {
      ...
      const res = await fetch(
        "https://swiftshipbackend-production.up.railway.app/getactiveorders",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const [completedOrdersWindow, setCompletedOrdersWindow] =
    React.useState(false);
  const getCompletedOrders = async () => {
    try {
      const res = await fetch(
        "https://swiftshipbackend-production.up.railway.app/getcompletedorders",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        "https://swiftshipbackend-production.up.railway.app/updateUser",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...
            <div className="flex flex-col items-left justify-left min-h-screen">
              ...
              >
                  ...
                  {orders.map(
                    (order, index) =>
                      ...
                      order.OrderStatusId != 5 && (
                        <li
                          key={index}
                          className="border-b border-red-200 py-2"
                        >
                          Order #{order.UID} -{" "}
                          <a
                            href={`/track/${order.UID + 1234567890}`}
                ...
                <button
                  ...
                  onClick={() =>
                    setCompletedOrdersWindow(!completedOrdersWindow)
                  }
                >
                ...
                <form className="space-y-4" onSubmit={handleSubmit}>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setUser1({ ...user1, mobile: e.target.value })
                      }
                      type="tel"
                      id="phone"
                      className="w-full px-3 py-2 border border-gray-300 rounded-md"
